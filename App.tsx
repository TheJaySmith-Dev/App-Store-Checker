...
import { CheckerUI } from './components/CheckerUI.tsx';
  ...
  const handleCheck = async (appId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch availability data. The server may have timed out or reached a limit.');
      }
      ...
      <main className="space-y-12">
        ...
        {!results && !isLoading && !error && (
          <div className="flex flex-col items-center justify-center py-20 opacity-30 grayscale pointer-events-none">
            ...
            <p className="text-xl font-bold">Awaiting Target ID...</p>
          </div>
        )}
      </main>
